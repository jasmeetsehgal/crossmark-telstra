            ...
            api.onBeforeSeek(function (e, index) {
                setTimeout(function () {
                    if (!e.isDefaultPrevented()) {
                        var i = index / size,
							 el = els().eq(i);
                        var navlen = navi.length == 1 ? 1 : navi.length + 1;
                        //console.log(i * (navlen), " a ", index, size, navi.length);
                        if (el.length) { els().removeClass(cls).eq(i * (navlen)).addClass(cls); }
                    }
                }, 1);
            });
